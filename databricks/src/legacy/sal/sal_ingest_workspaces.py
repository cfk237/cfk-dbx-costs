...
# DBTITLE 1,SAL Ingest - Workspace
# MAGIC %md
# MAGIC ## SAL Ingest — Workspace
# MAGIC
# MAGIC Incrementally ingests Parquet files produced by `raw_extract_workspaces.py` into the
# MAGIC `sal_workspaces` SAL Delta table using **Auto Loader** (`cloudFiles`).
# MAGIC
# MAGIC | Item | Value |
# MAGIC |---|---|
# MAGIC | Source | `{volume_base_path}/raw/system_access_workspaces_latest/` |
# MAGIC | Target | `{catalog}.databricks.sal_workspaces` |
# MAGIC | Checkpoint | `{volume_base_path}/checkpoints/access_workspaces_latest/autoloader/` |
# MAGIC
# MAGIC **Note:** `system.access.workspaces_latest` is a global account-level table.
# MAGIC Files are written by a single central `raw_extract_workspaces.py` run into a flat
...
target_table    = f"{catalog}.{schema}.sal_workspaces"
